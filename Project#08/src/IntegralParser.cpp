...
torch::Tensor IntegralParser::get_S() const { return S; }
torch::Tensor IntegralParser::get_T() const { return T; }
torch::Tensor IntegralParser::get_V() const { return V; }
...
// Matrix(mu_i, nu_j)
torch::Tensor IntegralParser::one_elec_parser(const std::string &fname) const {
  auto file = get_file(fname);
  ...
  // Fill matrix
  torch::Tensor mat = torch::zeros({n, n}, torch::dtype(torch::kF64));
  for (size_t i = 0; i < vals.size(); ++i) {
    // change to zero indexed
    mat.index_put_({mus[i] - 1, nus[i] - 1}, vals[i]);
  }

  // Symmetrise
  mat.add_(mat.clone().transpose(0, 1));
  mat.diagonal() /= 2;
...
void IntegralParser::orthogonalize_S() {
  auto [vals, vecs] = torch::linalg_eigh(this->S);

  S = torch::einsum("ij,jk,lk->il",
                    {vecs, torch::diag(vals.pow_(-1).sqrt()), vecs});
}
